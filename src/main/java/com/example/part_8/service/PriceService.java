...
import com.example.part_8.dto.MessageDTO.Type;
...
public class PriceService {
	...
	// NOTE: Use MessageMapper utility class for message validation and
	// transformation

	public Flux<MessageDTO<Float>> calculatePriceAndAveragePrice(Flux<Map<String, Object>> input,
			Flux<Long> averagesTimeInterval) {

		return input.doOnNext(event -> logger.fine("Incoming event: " + event))
				.transform(this::selectOnlyPriceUpdateEvents).transform(this::currentPrice)
				.doOnNext(event -> logger.fine("Price event: " + event))
				.transform(mainFlow -> Flux.merge(mainFlow, averagePrice(averagesTimeInterval, mainFlow)));
	}
	...
	// HINT consider corner case when client did not send any info about interval
	// (add initial interval (mergeWith(...)))
	...
	// 2.1) TODO hold latest event with currency price to send latest price if new
	// incoming event does not include price info
	// HINT use .scan() + HashMap + .filter to ensure that after proper scan passed
	// map include Price
	// 2.2) TODO map to Statistic message using MessageDTO#price()

	// Visible for testing
	// TODO: Remove as should be implemented by trainees
	Flux<MessageDTO<Float>> averagePrice(Flux<Long> requestedInterval, Flux<MessageDTO<Float>> priceData) {
		return requestedInterval.mergeWith(Mono.just(DEFAULT_AVG_PRICE_INTERVAL))
				.switchMap(l -> priceData.window(Duration.ofSeconds(l)))
				.flatMap(flux -> flux.groupBy(MessageDTO::getCurrency))
				.flatMap(flux -> flux.reduce(Sum.empty(), (a, v) -> a.add(v.getData()))
						.map(sum -> MessageDTO.avg(sum.getValue() / sum.getCounter(), flux.key(), "Average")));
	}
